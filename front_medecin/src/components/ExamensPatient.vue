      ...
      <div class="p-2 bg-white rounded-lg">
        <div class="h-full">
          ...
          <div class="flex flex-col gap-2 overflow-auto mt-5">
            <PrimaryButton v-for="(examen, index) in examens" :key="index" @click="selectedExamen(examen.id_examen)"
              class='mb-2 !shadow-none' :buttonClass="{'bg-neutral-200 bg-image-none text-black':examen.id_examen!=examSelected, 'text-white':examen.id_examen==examSelected}">{{ examen.date }} <br> {{ examen.motif }}</PrimaryButton>
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-col h-full col-span-3 gap-6 p-2">
          <div v-if="this.examSelected != null" class="grid w-full h-full grid-cols-3 gap-2">
            <div class="w-full h-full col-span-2 row-span-1 pb-5">
              ...
              <p class="w-full h-full bg-mercury-200 rounded-lg py-1.5 px-2">{{ examens.find(examen => examen.id_examen == examSelected).motif }}
              </p>
            </div>
          ...
          <div class="flex flex-col h-full pb-2" v-else>
            ...
            <div class="grid w-full h-full grid-cols-3 gap-2 flex-grow">
              <div class="w-full h-full col-span-2 row-span-1 pb-5">
                ...
                <textarea style="resize: none;" class="w-full h-full bg-mercury-200 rounded-lg px-2 py-1.5" name="motif"></textarea>
              </div>
              <div class="w-full h-full col-span-1 row-span-3 pb-5">
                <h5 class="w-full h-fit font-semibold text-lg">Diagnostic :</h5>
                <textarea style="resize: none;" class="w-full h-full bg-mercury-200 rounded-lg px-2 py-1.5" name="diagnostic"></textarea>
              </div>
              <div class="w-full h-full col-span-2 row-span-2 pb-5">
                <h5 class="w-full h-fit font-semibold text-lg">Résultats d'analyse :</h5>
                <textarea style="resize: none;" class="w-full h-full bg-mercury-200 rounded-lg px-2 py-1.5" name="resultat"></textarea>
              </div>
              <div class="w-full h-full col-span-2 row-span-2 pb-5">
                <h5 class="w-full h-fit font-semibold text-lg">Conclusion :</h5>
                <textarea style="resize: none;" class="w-full h-full bg-mercury-200 rounded-lg px-2 py-1.5" name="conclusion"></textarea>
              </div>
              <div class="w-full h-full col-span-1 row-span-2 pb-5">
                <h5 class="relative font-semibold text-lg">Observation :</h5>
                <textarea style="resize: none;" class="w-full h-full bg-mercury-200 rounded-lg px-2 py-1.5" name="observation"></textarea>
              </div>
            </div>
          </div>
        </div>
